...
bool ct_eq(const unsigned char* a, const unsigned char* b, std::size_t n) {
    unsigned char d = 0;
    for (std::size_t i = 0; i < n; ++i) d |= static_cast<unsigned char>(a[i] ^ b[i]);
    return d == 0;
}
...
std::string b64url_encode(const unsigned char* data, std::size_t len) {
    ...
    if (i < len) {
        ...
        out += B64URL[(v >> 18) & 0x3f];
        out += B64URL[(v >> 12) & 0x3f];
        if (i + 1 < len) out += B64URL[(v >> 6) & 0x3f];
    }
    return out;
}
...
std::string jwt_sign_hs256(std::string_view payload_json, std::string_view secret) {
    ...
    if (!hmac_sha256(secret, msg, sig)) return "";
...
std::optional<std::string> jwt_verify_hs256(
    std::string_view token, std::string_view secret, std::int64_t now_unix) {
    if (token.empty() || secret.empty()) return std::nullopt;
    auto d1 = token.find('.');
    ...
    if (header_b64 != HS256_HEADER_B64) return std::nullopt;

    std::string msg(token.substr(0, d2));
    unsigned char sig[32];
    if (!hmac_sha256(secret, msg, sig)) return std::nullopt;
    std::string expected_sig = b64url_encode(sig, 32);
    if (expected_sig.size() != sig_b64.size()) return std::nullopt;
    ...
    if (exp > 0 && now_unix >= exp) return std::nullopt;
    return payload;
}
...
std::optional<std::string> cookie_get_session(std::string_view hdr) {
    ...
    while (i < hdr.size()) {
        ...
        if (val_end == std::string_view::npos) val_end = hdr.size();
...
bool roles_contains_any(std::string_view roles_json,
    ...
    while (i < roles_json.size()) {
        if (roles_json[i] == '"') {
            ...
            for (auto w : wanted) if (tok == w) return true;
            i = end + 1;
        } else {
            ++i;
        }
