...
static int hmac_sha256(const char *secret, size_t secret_len, const char *msg, size_t msg_len,
                       unsigned char out[32]) {
    unsigned int outlen = 32;
    unsigned char *r = HMAC(EVP_sha256(), (const unsigned char *)secret, (int)secret_len,
                            (const unsigned char *)msg, msg_len, out, &outlen);
...
int jwt_sign_hs256(const char *payload_json, size_t payload_len, const char *secret,
    ...
    if (!payload_b64)
        return -1;
    int plen = b64url_encode((const unsigned char *)payload_json, payload_len, payload_b64,
                             payload_b64_cap);
    if (plen < 0) {
        free(payload_b64);
        return -1;
...
int jwt_verify_hs256(const char *token, size_t token_len, const char *secret, size_t secret_len,
    ...
    unsigned char sig[32];
    if (hmac_sha256(secret, secret_len, token, msg_len, sig) != 0)
        return -1;
    char expected_sig_b64[64];
    int exp_len = b64url_encode(sig, 32, expected_sig_b64, sizeof(expected_sig_b64));
    if (exp_len < 0)
        return -1;
    if ((size_t)exp_len != sig_b64_len)
        return -1;
    if (ct_memcmp(expected_sig_b64, sig_b64, sig_b64_len) != 0)
        return -1;

    /* decode payload */
    int decoded =
        b64url_decode(payload_b64, payload_b64_len, (unsigned char *)payload_out, payload_cap - 1);
    if (decoded < 0)
        return -1;
    ...
    if (exp_key) {
        ...
        if (colon) {
            ...
            if (exp_val > 0 && current_time_unix >= exp_val)
                return -1;
        }
    }
    return 0;
...
bool roles_contains_any(const char *roles_json, size_t roles_len, const char **wanted,
                        size_t wanted_count) {
    if (!roles_json || roles_len < 2)
        return false;
    ...
    while (p < end) {
        if (*p == '"') {
            ...
            for (size_t i = 0; i < wanted_count; i++) {
                if (slice_eq(p, (size_t)(q - p), wanted[i]))
                    return true;
            }
            p = q + 1;
        } else {
