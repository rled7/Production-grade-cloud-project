...
static char *build_conninfo(const char *host, const char *port,
...
bool db_ensure(db_ctx_t *ctx) {
    if (!ctx) return false;
    if (ctx->conn && PQstatus(ctx->conn) == CONNECTION_OK) return true;
    if (ctx->conn) {
        PQreset(ctx->conn);
        if (PQstatus(ctx->conn) == CONNECTION_OK) return true;
        PQfinish(ctx->conn);
        ctx->conn = NULL;
    }
    ctx->conn = PQconnectdb(ctx->conninfo);
...
db_status_t db_insert(db_ctx_t *ctx, const char *content, db_row_t *out) {
    if (!out || !content) return DB_BAD_INPUT;
    memset(out, 0, sizeof(*out));
    if (!db_ensure(ctx)) return DB_UNAVAILABLE;
